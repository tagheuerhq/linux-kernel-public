...
/* start jianghan@20210203 add for B841 evt */
u8 fts_ic_type = 0;
extern char *saved_command_line;
/* end jianghan@20210203 add for B841 evt */
...
void fts_reset_retry(void)
{
	int i = 0;
	u8 temp_buff = 0;
	u8 write_buf_0[] = {0x55,0xaa};
	u8 write_buf_1[] = {0x07,0xF8, 0x80, 0x40, 0x80, 0x00, 0x80, 0x00};
	u8 write_buf_2[] = {0x0a,0xf5};

	FTS_INFO("enter fts_reset_retry\n");
	for(i = 0; i < 2; i++){
		fts_reset_proc(0);
		fts_write(write_buf_0, sizeof(write_buf_0));
		fts_write(write_buf_1, sizeof(write_buf_1));
		fts_write(write_buf_2, sizeof(write_buf_2));

		msleep(50);
		fts_read_reg(0xa3, &temp_buff);
		if(temp_buff == 0x23){
			FTS_INFO("write fts_reset_retry success!!!\n");
			break;
		}
	}
	if(i >= 2){
		fts_reset_proc(0);
		FTS_INFO("write fts_reset_retry fail!!!\n");
	}
}
...
#ifdef CONFIG_ENABEL_TOUCH_WAKEUP
        if(data->aod_suspended == true){
            data->aod_suspended = false;
...
#if FTS_RESET_RETRY
	if(buf[1] == 0x9e){
		FTS_INFO("read touchdata buf[0] = 0x9e");
		fts_reset_retry();
		return 2;
	}
#endif
...
/* start jianghan@20200918 add for palm mode */
    if((buf[1]&0xFB) == 0x9B){
...
/* end jianghan@20200918 add for palm mode */
...
static int fts_irq_registration(struct fts_ts_data *ts_data)
{
    ...
    FTS_INFO_OTHER("irq:%d, flag:%x", ts_data->irq, pdata->irq_gpio_flags);
...
#if FTS_POWER_SOURCE_CUST_EN
    fts_power_source_init(ts);
#endif

    if(ts->disable_touch == 1){
        fts_irq_enable();
        ts->disable_touch = 0;
    }
...
static void fts_ts_close(struct input_dev *input_dev)
{
    ...
    if(ts->screen_now == FB_BLANK_UNBLANK){
        ts->disable_touch = 1;
    }
...
#if FTS_POWER_SOURCE_CUST_EN
    fts_power_source_exit(ts);
#endif

    FTS_FUNC_EXIT();
}

int fts_set_reset_proc(int reset_gpio_val)
{
    if(fts_data == NULL){
        FTS_ERROR("fts_data is null !");
        return -1;
    }else{
        if(reset_gpio_val != 0 && reset_gpio_val != 1){
            FTS_ERROR("reset_gpio_val is wrong !");
            return -1;
        }
        if(reset_gpio_val == 0){
            gpio_direction_output(fts_data->pdata->reset_gpio, 0);
...
EXPORT_SYMBOL_GPL(fts_set_reset_proc);
#endif
...
static int fts_power_source_ctrl(struct fts_ts_data *ts_data, int enable)
{
    ...
    if (enable) {
        if (ts_data->power_disabled) {
            ...
            /* reset_gpio = 0 --> 1  for touch wakeup*/
    #ifdef CONFIG_ENABEL_TOUCH_WAKEUP
        if((ts_data->disable_touch == 1) && (gpio_get_value(ts_data->pdata->reset_gpio) == 0)){
...
#ifdef CONFIG_ENABEL_TOUCH_WAKEUP
            msleep(2);
            if(ts_data->screen_now == FB_BLANK_POWERDOWN){
                gpio_direction_output(ts_data->pdata->lcd_reset_gpio, 0);
                msleep(2);
                gpio_direction_output(ts_data->pdata->vci_gpio, 0);
    	    }
#endif
...
static int fts_power_source_init(struct fts_ts_data *ts_data)
{
    ...
    FTS_FUNC_ENTER();
    /* vdd -> vdd_ana */
    ts_data->vdd = regulator_get(ts_data->dev, "vdd_ana");
    if (IS_ERR_OR_NULL(ts_data->vdd)) {
...
#ifdef CONFIG_ENABEL_TOUCH_WAKEUP
    if(ts_data->first_close != false){
        fts_power_source_ctrl(ts_data, DISABLE);
    }else{
        ts_data->first_close = true;
        FTS_DEBUG("start no set tprst");
    }
#endif
                             ...
                             struct fts_ts_platform_data *pdata)
{
    ...
    FTS_INFO_OTHER("display x(%d %d) y(%d %d)", pdata->x_min, pdata->x_max,
...
static int fts_parse_dt(struct device *dev, struct fts_ts_platform_data *pdata)
{
...
#ifdef CONFIG_ENABEL_TOUCH_WAKEUP
    pdata->vci_gpio = of_get_named_gpio_flags(np,"focaltech,platform-enable-gpio",
                        0, &pdata->vci_gpio_flags);
    if (pdata->vci_gpio < 0){
        FTS_ERROR("Unable to get vci_gpio");
    }

    pdata->lcd_reset_gpio = of_get_named_gpio_flags(np, "focaltech,lcd-reset-gpio",
                        0, &pdata->lcd_reset_gpio_flags);
    ...
    FTS_INFO_OTHER("max touch number:%d, irq gpio:%d, reset gpio:%d",
...
#if FTS_RESET_RETRY
{
    ...
    fts_read_reg(0x90, &temp_buff);
	if(temp_buff != 0x99){
		FTS_INFO("first read 0x99 = %x\n", temp_buff);
		for(i = 0; i < 100; i++){
			mdelay(10);
			fts_read_reg(0x90, &temp_buff);
			if(temp_buff == 0x99){
				fts_reset_retry();
				FTS_INFO("waite for 0x99 %d tiem\n", i);
				break;
			}
		}
		if(i >= 100)
			FTS_INFO("waite for 0x99 timeout\n");
	} else{
		FTS_INFO("first read 0x99 success");
		fts_reset_retry();
	}
}
#endif
...
static int fts_ts_suspend(struct device *dev)
{
    ...
    if (ts_data->gesture_mode) {
        ...
        fts_irq_disable();
#ifndef CONFIG_ENABEL_TOUCH_WAKEUP
        FTS_INFO("make TP enter into sleep mode");
        ret = fts_write_reg(FTS_REG_POWER_MODE, FTS_REG_POWER_MODE_SLEEP);
#endif
        if (!ts_data->ic_info.is_incell) {
    ...
    if (device_may_wakeup(&ts_data->client->dev)){
        FTS_INFO("Device may wakeup");
        if (ts_data->irq_wake == false){
            enable_irq_wake(ts_data->irq);
            ts_data->irq_wake = true;
        }
    }
    else{
        FTS_INFO("Device not wakeup");
    }
    fts_irq_enable();
    ts_data->aod_suspended = true;
#endif
...
#if FTS_RESET_RETRY
{
	...
	fts_read_reg(0x90, &temp_buff);
	if(temp_buff == 0x99){
		FTS_INFO("tpd_resume read 0x99 success");
		fts_reset_retry();
	}
}
#endif
...
#ifdef CONFIG_ENABEL_TOUCH_WAKEUP
    ts_data->irq_wake = false;
    ts_data->first_close = false;
    ts_data->aod_suspended = false;
    ts_data->disable_touch = 0;
...
static int __init fts_ts_init(void)
{
    ...
    char *lcd_string;
    char lcd_id[100];
    char needle[64]="mdss_dsi_auo_ft2388_454p_amoled_cmd";
    char min_needle[64]="mdss_dsi_auo";
    len = strlen(needle);
    memset(lcd_id, 0, sizeof(lcd_id));
    lcd_string = strstr(saved_command_line,min_needle);
    strncpy(lcd_id,lcd_string,len);

    FTS_FUNC_ENTER();

    if(IS_ERR_OR_NULL(lcd_id)){
        FTS_INFO("get cmdline fail");
    }else{
        if(strcmp(lcd_id,"mdss_dsi_auo_ft2388_454p_amoled_cmd") == 0){
            fts_ic_type = TP_VENDOR_AA;
        }else if(strcmp(lcd_id,"mdss_dsi_auo_ft2388_454p_evt_amoled") == 0){
            fts_ic_type = TP_VENDOR_AB;
            FTS_INFO("not raydium");
            return -ENODEV;
        }else{
            FTS_INFO("Unknown touch");
            return -ENODEV;
        }
    }
    /* end jianghan@20210203 add for B841 evt */
