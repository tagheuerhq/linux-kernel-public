...
int fts_fwupg_reset_in_boot(void)
{
    ...
    FTS_INFO_OTHER("reset in boot environment");
    ret = fts_write(&cmd, 1);
    if (ret < 0) {
        FTS_ERROR("pram/rom/bootloader reset cmd write fail");
        return ret;
...
int fts_fwupg_enter_into_boot(void)
{
    ...
    FTS_INFO_OTHER("***********enter into pramboot/bootloader***********");
...
int fts_fwupg_ecc_cal(u32 saddr, u32 len)
{
    ...
    FTS_INFO_OTHER( "**********read out checksum**********");
    if ((NULL == upg) || (NULL == upg->func)) {
        FTS_ERROR("upgrade/func is null");
        return -EINVAL;
    }
    ...
    FTS_INFO_OTHER("ecc calc num:%d, remainder:%d", packet_num, remainder);
    ...
    u32 delay)
{
    ...
    FTS_INFO_OTHER( "**********write data to flash**********");
    if ((!upg) || (!upg->func || !buf || !len)) {
        FTS_ERROR("upgrade/func/buf/len is invalid");
        return -EINVAL;
    }

    FTS_INFO_OTHER("data buf start addr=0x%x, len=0x%x", saddr, len);
    packet_number = len / FTS_FLASH_PACKET_LENGTH;
    remainder = len % FTS_FLASH_PACKET_LENGTH;
    if (remainder > 0)
        packet_number++;
...
int fts_fwupg_upgrade(struct fts_upgrade *upg)
{
    ...
    FTS_INFO_OTHER("fw auto upgrade function");
    if ((NULL == upg) || (NULL == upg->func)) {
...
static void fts_fwupg_auto_upgrade(struct fts_upgrade *upg)
{
    ...
    FTS_INFO_OTHER("********************FTS enter upgrade********************");
    if (!upg || !upg->ts_data) {
    ...
    FTS_INFO_OTHER("********************FTS exit upgrade********************");
}
...
/* start jianghan@20210203 add for B841 evt */
    if(fts_ic_type == TP_VENDOR_AA){
        upg->module_info = info;
    }else if(fts_ic_type == TP_VENDOR_AB){
        info = &module_list[1];
        upg->module_info = info;
    }
...
static int fts_fwupg_get_fw_file(struct fts_upgrade *upg)
{
    ...
    FTS_INFO_OTHER("upgrade fw file len:%d", upg->fw_length);
...
/* end jianghan@20200909 add for tp_info */ 
    ...
    FTS_INFO_OTHER("fw upgrade work function");
    if (!upg || !upg->ts_data) {
        FTS_ERROR("upg/ts_data is null");
        return ;
    }
...
/* start jianghan@20200909 add for tp_info */
    ...
    tmp_ver = fts_read_reg(FTS_REG_FW_VER, &fw_ver);
    tmp_ven = fts_read_reg(FTS_REG_VENDOR_ID, &fw_ven);
	memset(tp_version, 0, sizeof(tp_version));
	sprintf(tp_version, "[FW]%02X.%02X,[IC]FT2388",
		fw_ver,fw_ven);
        init_tp_fm_info(0, tp_version, "Focaltech");
#endif
/* end jianghan@20200909 add for tp_info */ 
    }
...
int fts_fwupg_init(struct fts_ts_data *ts_data)
{
    ...
    FTS_INFO_OTHER("fw upgrade init function");
