...
/*****************************************************************************
*
* File Name: focaltech_upgrade_ft2388.c
*
...
static int ft2388_pram_ecc_cal_algo(u32 start_addr, u32 ecc_length)
{
    int ret = 0;
    int i = 0;
    int ecc = 0;
    ...
    FTS_INFO_OTHER("read out pramboot checksum");
...
static int ft2388_get_boot_state(enum FW_STATUS *fw_sts)
{
    int ret = 0;
    u8 cmd[4] = { 0 };
    u32 cmd_len = 0;
...
static int ft2388_pram_start(void)
{
    u8 cmd = FTS_ROMBOOT_CMD_START_APP;
    int ret = 0;

    FTS_INFO_OTHER("remap to start pramboot");
...
static bool ft2388_check_state(enum FW_STATUS rstate)
{
    int ret = 0;
    int i = 0;
    ...
    for (i = 0; i < FTS_UPGRADE_LOOP; i++) {
        ret = ft2388_get_boot_state(&cstate);
...
static int ft2388_pram_write_buf(u8 *buf, u32 len)
{
    int ret = 0;
    int ecc_flag = 0;
    u32 i = 0;
    ...
    FTS_INFO_OTHER("write pramboot to pram");

    FTS_INFO_OTHER("pramboot len=%d", len);
...
static int ft2388_pram_write_remap(void)
{
    int ret = 0;
    int ecc_in_host = 0;
    int ecc_in_tp = 0;
    ...
    FTS_INFO_OTHER("write pram and remap");
    ...
    ecc_in_host = ft2388_pram_write_buf(pb_buf, pb_len);
    if (ecc_in_host < 0) {
        FTS_ERROR( "write pramboot fail");
        return ecc_in_host;
    }
    /* read out checksum */
    ecc_in_tp = ft2388_pram_ecc_cal_algo(0, pb_len);
    if (ecc_in_tp < 0) {
        FTS_ERROR( "read pramboot ecc fail");
        return ecc_in_tp;
    }

    FTS_INFO_OTHER("pram ecc in tp:%x, host:%x", ecc_in_tp, ecc_in_host);
...
static int ft2388_reset_to_romboot(void)
{
    int ret = 0;
    int i = 0;
    ...
    for (i = 0; i < FTS_UPGRADE_LOOP; i++) {
        ret = ft2388_get_boot_state(&state);
...
static int fts_ft2388_write_pramboot_private(void)
{
    int ret = 0;
    bool state = 0;

    FTS_INFO_OTHER("**********pram write and init**********");

    FTS_DEBUG("check whether tp is in romboot or not ");
    /* need reset to romboot when non-romboot state */
    state = ft2388_check_state(FTS_RUN_IN_ROM);
    if (!state) {
        FTS_INFO("tp isn't in romboot, need send reset to romboot");
        ret = ft2388_reset_to_romboot();
        if (ret < 0) {
            FTS_ERROR("reset to romboot fail");
            return ret;
        }
    ...
    state = ft2388_check_state(FTS_RUN_IN_PRAM);
    if (!state) {
...
struct upgrade_func upgrade_func_ft2388 = {
    ...
    .hid_supported = false,
    .write_pramboot_private = fts_ft2388_write_pramboot_private,
