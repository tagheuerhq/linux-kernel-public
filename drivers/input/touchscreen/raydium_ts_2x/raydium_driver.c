...
/* jianghan@20210610 add for touch-wake Begin*/
extern char *saved_command_line;
/* jianghan@20210610 add for touch-wake End*/
...
#endif
	g_u8_wakeupflag = false;
}
#ifdef CONFIG_RAYDIUM_TOUCH_WAKEUP
int raydium_set_reset_gpio(int raydium_reset_gpio_val)
{
    if(g_raydium_ts == NULL){
        pr_debug("[touch]g_raydium_ts is null!");
        return -1;
    }else{
        if(raydium_reset_gpio_val != 0 && raydium_reset_gpio_val != 1){
            pr_debug("reset_gpio_val is wrong !");
            return -1;
        }
        if(raydium_reset_gpio_val == 0)
...
EXPORT_SYMBOL_GPL(raydium_set_reset_gpio);
#endif
...
void raydium_irq_control(bool enable)
{
	if (enable) {
		if (g_raydium_ts->irq_enabled) {
			...
			dev_dbg(&g_raydium_ts->client->dev,
...
static void raydium_work_handler(struct work_struct *work)
{
	...
	if (g_u8_i2c_mode == PDA2_MODE) {
		...
		if (i32_ret < 0) {
			pr_debug("[touch]%s, read_touchdata error, ret:%d\n",
				__func__, i32_ret);
			return;
		}
	}
	...
	if (g_raydium_ts->blank == FB_BLANK_UNBLANK) {
		if (u8_tp_status[POS_GES_STATUS] == RAD_PALM_ENABLE) {
			...
			g_raydium_ts->blank == FB_BLANK_NORMAL) {
	...
	if (g_u8_i2c_mode == PDA2_MODE) {
		...
		*(unsigned int *)u8_buf =
		...
		if (i32_ret < 0)
			goto exit_error;
		if (u8_buf[3] != 0xF3) {
			pr_debug("[touch]PDA2 read i2c fail\n");
			g_u8_i2c_mode = PDA_MODE;
...
static void raydium_ts_do_suspend(void)
{
	...
	if (g_raydium_ts->is_suspend == 1) {
		pr_debug("[touch]Already in suspend state\n");
		return;
	}
	...
	if (!cancel_work_sync(&g_raydium_ts->work))
		pr_debug("[touch]workqueue is empty!\n");
...
static void raydium_ts_do_resume(void)
{
	...
	if (g_raydium_ts->is_suspend == 0) {
		pr_debug("[touch]Already in resume state\n");
		return;
	}
	...
	/* clear workqueue*/
	if (!cancel_work_sync(&g_raydium_ts->work))
		pr_debug("[ raydium ]workqueue is empty!\n");
#ifdef ESD_SOLUTION_EN
				...
				void *data)
{
	...
	if (evdata && evdata->data && event == FB_EVENT_BLANK &&
		...
		case FB_BLANK_NORMAL:
			pr_debug("[touch]FB_BLANK_NORMAL\n");
#ifdef GESTURE_EN
...
static void raydium_input_set(struct input_dev *input_dev)
{
	...
	input_dev->name = "raydium_ts_custom";/*name need same with .idc*/
			    ...
			    const struct i2c_device_id *id)
{
	...
	if ((pdata->num_max_touches > MAX_TOUCH_NUM) || 
		(pdata->num_max_touches == 0))
		g_raydium_ts->u8_max_touchs = MAX_TOUCH_NUM;
	else
	...
	pr_debug("[touch]RAD Touch driver ver :0x%X\n", g_u32_driver_version);
...
/* jianghan@20210610 add for touch-wake Begin*/
	char *lcd_name;
	char name[64] = "auo_rm6d020";
	lcd_name = strstr(saved_command_line,name);
	if(IS_ERR_OR_NULL(lcd_name)){
		pr_err("[touch]get cmdline fail or lcd not match tp");
		return -ENODEV;
	}
/* jianghan@20210610 add for touch-wake End*/
