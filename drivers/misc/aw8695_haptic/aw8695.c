...
#define AW8695_DRIVER_VERSION "v1.4.12"
...
static void aw8695_ram_loaded(const struct firmware *cont, void *context)
{
	...
	queue_delayed_work(aw8695->work_queue, &aw8695->ram_work,
			   msecs_to_jiffies(ram_timer_val));						  
			pr_debug("%s:start hrtimer:load_cont%d\n", __func__,
				load_cont);
		}
#endif
		return;
...
static int aw8695_haptic_juge_RTP_is_going_on(struct aw8695 *aw8695)
{
	...
	if (pre_reg_val != reg_val)
		pr_debug("%sAW8695_REG_SYSCTRL 0x04==%02x rtp_state=%d\n",
...
static int aw8695_haptic_play_go(struct aw8695 *aw8695, bool flag)
{
	...
	if (flag) {
		...
		if (aw8695->interval_us < 2000) {
			pr_debug("aw8695->interval_us=%d < 2000\n",
				aw8695->interval_us);
			mdelay(2);
		}
		aw8695_i2c_write_bits(aw8695, AW8695_REG_GO,
...
static int16_t aw8695_haptic_effect_strength(struct aw8695 *aw8695)
{
	pr_debug("%s enter\n", __func__);
	VIB_DEBUG("aw8695->play.vmax_mv =0x%x", aw8695->play.vmax_mv);
	switch (aw8695->play.vmax_mv) {
	case AW8695_LIGHT_MAGNITUDE:
		aw8695->level = 0x80;
		break;
	case AW8695_MEDIUM_MAGNITUDE:
		aw8695->level = 0x50;
		break;
	case AW8695_STRONG_MAGNITUDE:
		aw8695->level = 0x30;
		break;
	default:
		break;
	}
	VIB_DEBUG("aw8695->level =0x%x", aw8695->level);
	return 0;
}

/*static int aw8695_haptic_play_effect_seq(struct aw8695 *aw8695,
					 unsigned char flag)
{
	pr_debug("%s enter  \n", __func__);

	if (aw8695->effect_id > aw8695->info.effect_id_boundary)
		return 0;
	VIB_DEBUG("effect_id =%d activate_mode =%d", aw8695->effect_id,
		  aw8695->activate_mode);
	if (flag) {
		if (aw8695->activate_mode == AW8695_HAPTIC_ACTIVATE_RAM_MODE) {
			aw8695_haptic_set_wav_seq(aw8695, 0x00,
						  (char)aw8695->effect_id + 1);
			aw8695_haptic_set_wav_seq(aw8695, 0x01, 0x00);
			aw8695_haptic_set_wav_loop(aw8695, 0x00, 0x00);
			aw8695_haptic_play_mode(aw8695, AW8695_HAPTIC_RAM_MODE);
			aw8695_haptic_effect_strength(aw8695);
			aw8695_haptic_set_gain(aw8695, aw8695->level);
			aw8695_haptic_start(aw8695);
		}
		if (aw8695->activate_mode ==
		    AW8695_HAPTIC_ACTIVATE_RAM_LOOP_MODE) {
			aw8695_haptic_set_repeat_wav_seq(aw8695,
							 (aw8695->info. effect_id_boundary +1));
...
static void aw8695_haptic_upload_lra(struct aw8695 *aw8695, unsigned char flag)
{
	switch (flag) {
	case NORMAL_CALI:
		...
		break;
	case F0_CALI:
		VIB_DEBUG("f0_cali_lra=%d", aw8695->f0_calib_data);
		aw8695_i2c_write(aw8695, AW8695_REG_TRIM_LRA,
				 (char)aw8695->f0_calib_data);
		break;
	case OSC_CALI:
		VIB_DEBUG("rtp_cali_lra=%d", aw8695->lra_calib_data);
		aw8695_i2c_write(aw8695, AW8695_REG_TRIM_LRA,
...
#if 0
static int aw8695_clock_OSC_trim_calibration(unsigned long int theory_time,
...
static int aw8695_rtp_osc_calibration(struct aw8695 *aw8695)
{
	...
	pr_debug("%s aw8695->theory_time = %d\n", __func__, aw8695->theory_time);

	while (1) {
		if (!aw8695_haptic_rtp_get_fifo_afi(aw8695)) {
			/* pr_debug("%s !aw8695_haptic_rtp_get_fifo_afi done aw8695->rtp_cnt= %d\n", __func__,aw8695->rtp_cnt); */
			mutex_lock(&aw8695->rtp_lock);
...
static void aw8695_rtp_work_routine(struct work_struct *work)
{
	...
	pr_debug("%s enter\n", __func__);
	if ((aw8695->effect_id < aw8695->info.effect_id_boundary) &&
	    (aw8695->effect_id > aw8695->info.effect_max))
		return;
	VIB_DEBUG("effect_id =%d state=%d", aw8695->effect_id, aw8695->state);
	mutex_lock(&aw8695->lock);
	aw8695_haptic_upload_lra(aw8695, OSC_CALI);
	aw8695_haptic_set_rtp_aei(aw8695, false);
	aw8695_interrupt_clear(aw8695);
#if 0
	//wait for irq to exit
	atomic_set(&aw8695->exit_in_rtp_loop, 1);
	while (atomic_read(&aw8695->is_in_rtp_loop)) {
		pr_debug("%s  goint to waiting irq exit\n", __func__);
		ret =
		    wait_event_interruptible(aw8695->wait_q,
					     atomic_read(&aw8695->is_in_rtp_loop) == 0);
		pr_debug("%s  wakeup \n", __func__);
	if (ret == -ERESTARTSYS) {
			atomic_set(&aw8695->exit_in_rtp_loop, 0);
			wake_up_interruptible(&aw8695->stop_wait_q);
			mutex_unlock(&aw8695->lock);
	...
	if (aw8695->state) {
		aw8695_haptic_effect_strength(aw8695);
		aw8695->rtp_file_num = aw8695->effect_id - 20;
		VIB_DEBUG("aw8695->rtp_file_num =%d", aw8695->rtp_file_num);
		if (aw8695->rtp_file_num < 0)
			aw8695->rtp_file_num = 0;
		if (aw8695->rtp_file_num >
		    ((sizeof(aw8695_rtp_name) / AW8695_RTP_NAME_MAX) - 1))
			aw8695->rtp_file_num =
			    (sizeof(aw8695_rtp_name) / AW8695_RTP_NAME_MAX) - 1;
							   ...
							   *timer)
{
	...
	queue_work(aw8695->work_queue, &aw8695->haptic_audio.work);
				 ...
				 size_t len, loff_t *off)
{
	...
	case AW8695_HAPTIC_CMD_WRITE_REG:
		if (len > 2) {
			for (i = 0; i < len - 2; i++) {
				pr_debug("%s: write reg0x%02x=0x%02x\n",
...
{
	struct aw8695 *aw8695 = container_of(timer, struct aw8695,
					     stop_timer);
	int rc;

	pr_debug("%s enter\n", __func__);
	aw8695->play.length_us = 0;
	rc = aw8695_haptic_play_go(aw8695, false);	// qti_haptics_play(aw8695, false);
	if (rc < 0)
		dev_err(aw8695->dev, "Stop playing failed, rc=%d\n", rc);
...
static enum hrtimer_restart qti_hap_disable_timer(struct hrtimer *timer)
{
	...
	pr_debug("%s enter\n", __func__);
	rc = aw8695_haptic_play_go(aw8695, false);	//qti_haptics_module_en(aw8695, false);
	if (rc < 0)
		dev_err(aw8695->dev, "Disable haptics module failed, rc=%d\n",
			rc);
...
static enum hrtimer_restart aw8695_vibrator_timer_func(struct hrtimer *timer)
{
	...
	queue_work(aw8695->work_queue, &aw8695->vibrator_work);
...
static void aw8695_vibrator_work_routine(struct work_struct *work)
{
	...
	mutex_lock(&aw8695->lock);
	aw8695_haptic_upload_lra(aw8695, F0_CALI);
	aw8695_haptic_stop(aw8695);
	if (aw8695->state) {
		if (aw8695->activate_mode == AW8695_HAPTIC_ACTIVATE_RAM_MODE) {
			//aw8695_haptic_ram_vbat_comp(aw8695, true);
			//aw8695_haptic_play_repeat_seq(aw8695, true);
			//Daniel 20201015 modify start 
			aw8695_haptic_ram_vbat_comp(aw8695, false);
			aw8695_haptic_play_mode(aw8695, AW8695_HAPTIC_RAM_MODE);
			aw8695_haptic_start(aw8695);
			//Daniel 20201015 modify end
		}else if (aw8695->activate_mode ==
			   AW8695_HAPTIC_ACTIVATE_RAM_LOOP_MODE) {
			aw8695_haptic_set_repeat_wav_seq(aw8695, 4);
			aw8695_haptic_ram_vbat_comp(aw8695, true);
			//aw8695_haptic_play_effect_seq(aw8695, true);
			aw8695_haptic_play_repeat_seq(aw8695, true);
			hrtimer_start(&aw8695->timer,
				      ktime_set(aw8695->duration / 1000,
						(aw8695->duration % 1000) *
			   ...
			   struct device_node *np)
{
	...
	if (val != 0)
		pr_debug("%s vib_f0_trace_parameter not found\n", __func__);
	...
	if (val != 0)
		pr_debug("%s vib_bemf_config not found\n", __func__);
	...
	struct qti_hap_config *config = &aw8695->config;
	...
	//memcpy(aw8695->info.trig_config, trig_config, sizeof(trig_config));
	...
	aw8695->constant.pattern = devm_kcalloc(aw8695->dev,
						HAP_WAVEFORM_BUFFER_MAX,
						sizeof(u8), GFP_KERNEL);
	if (!aw8695->constant.pattern)
		return -ENOMEM;

	tmp = of_get_available_child_count(np);
	aw8695->predefined = devm_kcalloc(aw8695->dev, tmp,
					  sizeof(*aw8695->predefined),
					  GFP_KERNEL);
	if (!aw8695->predefined)
		return -ENOMEM;

	aw8695->effects_count = tmp;
	for_each_available_child_of_node(np, child_node) {
		effect = &aw8695->predefined[i++];
		...
		effect->pattern = devm_kcalloc(aw8695->dev,
	...
	for (j = 0; j < i; j++) {
		VIB_DEBUG("effect_id: %d", aw8695->predefined[j].id);
		VIB_DEBUG("vmax: %d mv", aw8695->predefined[j].vmax_mv);
		VIB_DEBUG("play_rate: %d us",
			  aw8695->predefined[j].play_rate_us);
		for (m = 0; m < aw8695->predefined[j].pattern_length; m++)
			VIB_DEBUG("pattern[%d]: 0x%x", m,
				  aw8695->predefined[j].pattern[m]);
		for (m = 0; m < aw8695->predefined[j].brake_pattern_length; m++)
			VIB_DEBUG("brake_pattern[%d]: 0x%x", m,
				 aw8695->predefined[j].brake[m]);
		VIB_DEBUG("brake_en: %d", aw8695->predefined[j].brake_en);
		VIB_DEBUG("wf_repeat_n: %d",aw8695->predefined[j].wf_repeat_n);
		VIB_DEBUG("wf_s_repeat_n: %d",
			  aw8695->predefined[j].wf_s_repeat_n);
		VIB_DEBUG("lra_auto_res_disable: %d",
			  aw8695->predefined[j].lra_auto_res_disable);
	}
	VIB_DEBUG("aw8695->effects_count: %d", aw8695->effects_count);
	VIB_DEBUG("aw8695->effect_id_boundary: %d",
		  aw8695->info.effect_id_boundary);
	VIB_DEBUG("aw8695->effect_max: %d", aw8695->info.effect_max);
	VIB_DEBUG("aw8695->info.cont_drv_lvl: %d", aw8695->info.cont_drv_lvl);
	VIB_DEBUG("aw8695->info.cont_drv_lvl_ov: %d",
		  aw8695->info.cont_drv_lvl_ov);
	VIB_DEBUG("aw8695->info.gain_flag: %d", aw8695->info.gain_flag);
	for (i = 0; i < 3; i++)
		for (j = 0; j < 5; j++)
			VIB_DEBUG("aw8695->info.trig_config[%d][%d]: %d",
				  i, j, aw8695->info.trig_config[i][j]);
	for (i = 0; i < 175; i++)
		VIB_DEBUG("aw8695->info.rtp_time[%d]: %d",
			  i, aw8695->info.rtp_time[i]);
	VIB_DEBUG("aw8695->info.parameter1: 0x%x", aw8695->info.parameter1);
...
static int aw8695_haptics_upload_effect(struct input_dev *dev,
					struct ff_effect *effect,
					struct ff_effect *old)
{
	struct aw8695 *aw8695 = input_get_drvdata(dev);
	struct qti_hap_play_info *play = &aw8695->play;
	...
	/*for osc calibration */
	if (aw8695->osc_cali_run != 0)
		return 0;

	if (hrtimer_active(&aw8695->timer)) {
		rem = hrtimer_get_remaining(&aw8695->timer);
	...
	aw8695->effect_type = effect->type;
	VIB_DEBUG("aw8695->effect_type= 0x%x", aw8695->effect_type);
	mutex_lock(&aw8695->lock);
	
#if 0
	while (atomic_read(&aw8695->exit_in_rtp_loop)) {
		pr_debug("goint to waiting rtp  exit\n");
		mutex_unlock(&aw8695->lock);
		ret =
		    wait_event_interruptible(aw8695->stop_wait_q,
					     atomic_read(&aw8695-> exit_in_rtp_loop) == 0);
		pr_debug("%s  wakeup \n", __func__);
		if (ret == -ERESTARTSYS) {
			mutex_unlock(&aw8695->lock);
			pr_err("%s wake up by signal return erro\n", __func__);
	...
	if (aw8695->effect_type == FF_CONSTANT) {
		VIB_DEBUG("effect_type is  FF_CONSTANT!");
		/*cont mode set duration */
		aw8695->duration = effect->replay.length;
		aw8695->activate_mode = AW8695_HAPTIC_ACTIVATE_RAM_LOOP_MODE;
		aw8695->effect_id = aw8695->info.effect_id_boundary;

	} else if (aw8695->effect_type == FF_PERIODIC) {
		if (aw8695->effects_count == 0) {
			mutex_unlock(&aw8695->lock);
			return -EINVAL;
	}
		...
		aw8695->effect_id = data[0];
		VIB_DEBUG("aw8695->effect_id =%d", aw8695->effect_id);
		play->vmax_mv = effect->u.periodic.magnitude;	/*vmax level */
		if (aw8695->info.gain_flag == 1)
			play->vmax_mv = AW8695_LIGHT_MAGNITUDE;
		VIB_DEBUG("aw8695->play.vmax_mv = 0x%x",
			  aw8695->play.vmax_mv);

		if (aw8695->effect_id < 0 ||
		    aw8695->effect_id > aw8695->info.effect_max) {
			mutex_unlock(&aw8695->lock);
			return 0;
	}
		...
		if (aw8695->effect_id < aw8695->info.effect_id_boundary) {
			...
			VIB_DEBUG
			    ("%s aw8695->effect_id=%d , aw8695->activate_mode = %d",
			     __func__, aw8695->effect_id,
			     aw8695->activate_mode);
			data[1] = aw8695->predefined[aw8695->effect_id].play_rate_us / 1000000;	/*second data */
			data[2] = aw8695->predefined[aw8695->effect_id].play_rate_us / 1000;	/*millisecond data */
			VIB_DEBUG
			    ("aw8695->predefined[aw8695->effect_id].play_rate_us/1000 = %d",
			     aw8695->predefined[aw8695->effect_id].play_rate_us / 1000);
	}
		if (aw8695->effect_id >= aw8695->info.effect_id_boundary) {
			aw8695->activate_mode = AW8695_HAPTIC_ACTIVATE_RTP_MODE;
			VIB_DEBUG
			    ("aw8695->effect_id=%d , aw8695->activate_mode = %d",
			     aw8695->effect_id, aw8695->activate_mode);
			data[1] = aw8695->info.rtp_time[aw8695->effect_id] / 1000;	/*second data */
			data[2] = aw8695->info.rtp_time[aw8695->effect_id];	/*millisecond data */
...
static int aw8695_haptics_playback(struct input_dev *dev, int effect_id,
				   int val)
{
	struct aw8695 *aw8695 = input_get_drvdata(dev);
	int rc = 0;
	...
	VIB_DEBUG("aw8695->effect_id=%d , aw8695->activate_mode = %d",
		  aw8695->effect_id, aw8695->activate_mode);
	...
	if (aw8695->effect_type == FF_CONSTANT &&
		...
		queue_work(aw8695->work_queue, &aw8695->rtp_work);
...
static int aw8695_haptics_erase(struct input_dev *dev, int effect_id)
{
	struct aw8695 *aw8695 = input_get_drvdata(dev);
	int rc = 0;
...
static int select_pin_ctl(struct aw8695 *aw8695, const char *name)
{
	size_t i;
	int rc;

	for (i = 0; i < ARRAY_SIZE(aw8695->pinctrl_state); i++) {
		const char *n = pctl_names[i];

		if (!strncmp(n, name, strlen(n))) {
			rc = pinctrl_select_state(aw8695->aw8695_pinctrl,
						  aw8695->pinctrl_state[i]);
			if (rc)
...
static int aw8695_set_interrupt(struct aw8695 *aw8695)
{
	int rc = select_pin_ctl(aw8695, "aw8695_interrupt_active");
	return rc;
}
#endif
...
static int aw8695_hw_reset(struct aw8695 *aw8695)
{
	//pr_debug("%s enter\n", __func__);
#ifdef ENABLE_PIN_CONTROL
	int rc = select_pin_ctl(aw8695, "aw8695_reset_active");
	msleep(5);
...
/* wangkunpeng@20201210 used to read f0 data calibration and begin */		
	aw8695_haptic_f0_calibration(aw8695);	
	mutex_unlock(&aw8695->lock);
	len +=
...
static ssize_t aw8695_osc_cali_show(struct device *dev,
				    struct device_attribute *attr, char *buf)
{
	struct aw8695 *aw8695 = dev_get_drvdata(dev);
	ssize_t len = 0;
...
static ssize_t aw8695_osc_save_show(struct device *dev,
				    struct device_attribute *attr, char *buf)
{
	struct aw8695 *aw8695 = dev_get_drvdata(dev);
	ssize_t len = 0;
...
static ssize_t aw8695_osc_save_store(struct device *dev,
				     struct device_attribute *attr,
				     const char *buf, size_t count)
{
	struct aw8695 *aw8695 = dev_get_drvdata(dev);
	...
	if (rc < 0)
		return rc;
	aw8695->lra_calib_data = val;
	return count;
}

static ssize_t aw8695_f0_save_show(struct device *dev,
					 struct device_attribute *attr,
					 char *buf)
{
	struct aw8695 *aw8695 = dev_get_drvdata(dev);
...
static ssize_t aw8695_f0_save_store(struct device *dev,
				    struct device_attribute *attr,
				    const char *buf, size_t count)
{
	struct aw8695 *aw8695 = dev_get_drvdata(dev);
...
static DEVICE_ATTR(osc_cali, S_IWUSR | S_IRUGO, aw8695_osc_cali_show, aw8695_osc_cali_store);
static DEVICE_ATTR(f0_save, S_IWUSR | S_IRUGO, aw8695_f0_save_show, aw8695_f0_save_store);
static DEVICE_ATTR(osc_save, S_IWUSR | S_IRUGO, aw8695_osc_save_show, aw8695_osc_save_store);
			    ...
			    const struct i2c_device_id *id)
{
	...
	aw8695->enable_pin_control = 0;
#ifdef ENABLE_PIN_CONTROL
	aw8695->aw8695_pinctrl = devm_pinctrl_get(&i2c->dev);
	if (IS_ERR(aw8695->aw8695_pinctrl)) {
		if (PTR_ERR(aw8695->aw8695_pinctrl) == -EPROBE_DEFER) {
			VIB_DEBUG("pinctrl not ready\n");
			rc = -EPROBE_DEFER;
			return rc;
		}
		VIB_DEBUG("Target does not use pinctrl\n");
		aw8695->aw8695_pinctrl = NULL;
		rc = -EINVAL;
		return rc;
	}
	for (i = 0; i < ARRAY_SIZE(aw8695->pinctrl_state); i++) {
		...
		struct pinctrl_state *state =
		    pinctrl_lookup_state(aw8695->aw8695_pinctrl, n);
		if (IS_ERR(state)) {
		...
		aw8695->pinctrl_state[i] = state;
		aw8695->enable_pin_control = 1;
		aw8695_set_interrupt(aw8695);
	}
#endif
	if (!aw8695->enable_pin_control) {
	...
	if (gpio_is_valid(aw8695->irq_gpio) &&
		...
		pr_debug("%s skipping IRQ registration\n", __func__);
		/* disable feature support if gpio was invalid */
		aw8695->flags |= AW8695_FLAG_SKIP_INTERRUPTS;
	}

	hrtimer_init(&aw8695->stop_timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
	aw8695->stop_timer.function = qti_hap_stop_timer;
	hrtimer_init(&aw8695->hap_disable_timer, CLOCK_MONOTONIC,
		     HRTIMER_MODE_REL);
	aw8695->hap_disable_timer.function = qti_hap_disable_timer;

	input_dev->name = "aw8695_haptic";
	input_set_drvdata(input_dev, aw8695);
	aw8695->input_dev = input_dev;
	...
	if (aw8695->effects_count + 1 > FF_EFFECT_COUNT_MAX)
		effect_count_max = aw8695->effects_count + 1;
	else
		effect_count_max = FF_EFFECT_COUNT_MAX;
	rc = input_ff_create(input_dev, effect_count_max);
	if (rc < 0) {
		dev_err(aw8695->dev, "create FF input device failed, rc=%d\n",
			rc);
		return rc;
	}

	ff = input_dev->ff;
	ff->upload = aw8695_haptics_upload_effect;
	ff->playback = aw8695_haptics_playback;
	ff->erase = aw8695_haptics_erase;
	ff->set_gain = aw8695_haptics_set_gain;
	rc = input_register_device(input_dev);
	if (rc < 0) {
		dev_err(aw8695->dev, "register input device failed, rc=%d\n",
			rc);
		goto destroy_ff;
	}
	dev_set_drvdata(&i2c->dev, aw8695);

	ret = sysfs_create_group(&i2c->dev.kobj, &aw8695_vibrator_attribute_group);
	if (ret < 0) {
		pr_debug("%s error creating sysfs attr files\n",
			 __func__);
		goto err_sysfs;
	}
	...
	g_aw8695 = aw8695;
	aw8695->work_queue =
	    create_singlethread_workqueue("aw8695_vibrator_work_queue");
	if (!aw8695->work_queue) {
		dev_err(&i2c->dev,
			"%s: Error creating aw8695_vibrator_work_queue\n",
			__func__);
		goto err_sysfs;
	}
 ...
 destroy_ff:
	input_ff_destroy(aw8695->input_dev);
 err_irq:
 err_id:
	if (gpio_is_valid(aw8695->irq_gpio))
...
static int aw8695_i2c_remove(struct i2c_client *i2c)
{
	...
	if (gpio_is_valid(aw8695->reset_gpio))
		devm_gpio_free(&i2c->dev, aw8695->reset_gpio);
	if (aw8695 != NULL) {
		flush_workqueue(aw8695->work_queue);
		destroy_workqueue(aw8695->work_queue);
	}
	devm_kfree(&i2c->dev, aw8695);
	aw8695 = NULL;
...
static int __init aw8695_i2c_init(void)
{
	...
	pr_debug("aw8695 driver version %s\n", AW8695_DRIVER_VERSION);
