...
static int aw8624_haptic_play_go(struct aw8624 *aw8624, bool flag)
{
	...
	if (flag == true) {
		...
		mdelay(2);//Daniel 20201126 modify
	} else {
		aw8624_i2c_write_bits(aw8624, AW8624_REG_GO,
				      AW8624_BIT_GO_MASK,
...
static int aw8624_haptic_get_f0(struct aw8624 *aw8624)
{
	...
	/* f0 driver level */
	aw8624_i2c_write(aw8624, AW8624_REG_DRV_LVL, aw8624->info.cont_drv_lvl);
	if (!aw8624->info.f0_pre) {
		pr_debug("%s:fail to get t_f0_ms\n", __func__);
		return 0;
	}
	/* f0 trace parameter */
	t_f0_ms = (1000 * 10) / aw8624->info.f0_pre;//4
	f0_pre_num = aw8624->info.f0_trace_parameter[0];//9
	f0_wait_num = aw8624->info.f0_trace_parameter[1];//3
	f0_trace_num = aw8624->info.f0_trace_parameter[3];//31
	...
	/*f0_repeat_num =
	    ((aw8624->info.parameter1 * aw8624->info.f0_pre * 100) / 5000 -
	     100 * (f0_pre_num + f0_wait_num + 2)) / (f0_trace_num +
						      f0_wait_num + 2);
	pr_debug("%s:first f0_repeat_num  = %d\n", __func__, f0_repeat_num);
	if (f0_repeat_num <= 0) {
		f0_repeat_num = aw8624->info.f0_trace_parameter[2];
	} else {
	...
	pr_debug("%s:finally f0_repeat_num  = %d\n", __func__, f0_repeat_num);
...
/* wangkunpeng@20201215 enable voltage gain compensation begin */					 
	...
	if (aw8624->state) {
		if (aw8624->activate_mode == AW8624_HAPTIC_ACTIVATE_RAM_MODE) {
			...
			//aw8624_haptic_play_effect_seq(aw8624, true);
			   ...
			   AW8624_HAPTIC_ACTIVATE_RAM_LOOP_MODE) {
			aw8624_haptic_set_repeat_wav_seq(aw8624, 4);			   
...
/* wangkunpeng@20201211 used to read f0 data calibration and begin */	
	aw8624_haptic_f0_calibration(aw8624);
/* end */				
	mutex_unlock(&aw8624->lock);
	len +=
